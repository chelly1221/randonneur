...
export async function GET(request: NextRequest) {
  ...
  const tsQuery = q
    .split(/\s+/)
    .filter(Boolean)
    .map((w) => w.replace(/[^a-zA-Z0-9가-힣\u3400-\u9FFF_]/g, ""))
    .filter((w) => w.length > 0)
    .map((w) => `${w}:*`)
    .join(" & ");
  ...
  if (type === "all" || type === "courses") {
    results.courses = await prisma.$queryRaw`
      SELECT id, slug, name, distance_km as "distanceKm", elevation_m as "elevationM",
  ...
  if (type === "all" || type === "reviews") {
    results.reviews = await prisma.$queryRaw`
      SELECT r.id, r.content, r.created_at as "createdAt",
             c.id as "courseId", c.slug as "courseSlug", c.name as "courseName",
